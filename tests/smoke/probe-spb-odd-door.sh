...
SERVER_DIR="${SERVER_DIR:-$ROOT_DIR/server-smoke-probe}"
...
DEFAULT_SERVER_PORT="$((25642 + (BASHPID % 1000)))"
...
mkdir -p "$SERVER_DIR/mods" "$UNIVERSE_DIR"
...
send_command 'fill 18 63 0 18 66 0 air'
wait_for_log 'Successfully filled [0-9]+ block\(s\)|No blocks were filled'
send_command 'fill 18 63 0 18 63 0 stone'
wait_for_log 'Successfully filled 1 block\(s\)'
...
send_command 'execute if block 18 64 0 russianmetro:spb_horizontal_elevator_door_odd run say probe-odd-block-ok'
send_command 'execute if block 18 64 0 russianmetro:spb_horizontal_elevator_door run say probe-even-block-at-lower'
send_command 'execute if block 18 64 0 air run say probe-air-lower'
send_command 'execute if block 18 65 0 russianmetro:spb_horizontal_elevator_door_odd run say probe-odd-upper-ok'
send_command 'execute if block 18 65 0 russianmetro:spb_horizontal_elevator_door run say probe-even-block-at-upper'
send_command 'execute if block 18 65 0 air run say probe-air-upper'

wait_for_log 'probe-(odd|even|air)'
send_command 'stop'
wait "$SERVER_PID"
SERVER_PID=""

echo "Probe complete. Log: $LOG_FILE"
